...
MODULE MODAL_DG_1D_GLOBALS
  ...
  ! COUNTERS
  INTEGER                                                                          :: ORD                                    ! INDEX OF THE DOFREE FOR LEGENDRE POLYNOMIALS
  ...
  INTEGER, PARAMETER                                                               :: WENO_SW    = 0                         ! 0 : WITHOUT WENO  / 1 : WITH WENO  
  ...
  ! DOFREES OF FREEDOM IN THE MOMENTS BY THE L2 PROJECTION
  REAL( 8 ), DIMENSION( 0 : COMMON_ORD, - I_BC + 1 : IMAX_MAX + I_BC )             :: OLD_DOF
  REAL( 8 ), DIMENSION( 0 : COMMON_ORD, 1 : IMAX_MAX )                             :: NEW_DOF

  ! CONVEX SUMMATION OF DOFREES OF FREEDOM AND LEGENDRE POLYNOMIALS
  REAL( 8 )                                                                        :: TEMP
...
PROGRAM MODAL_DG_1D
  ...
  DO I0 = 0, NLEV
      ...
      CALL GET_INITIAL( PI, COMMON_ORD, Q, M, I_BC, IMAX, X, DX, OLD_DOF )
      ...
      DO WHILE ( ( FLAG_0 < TIME_OUT ) .AND. ( N <= NMAX ) )
          ...
          CALL GET_RK( RK_ORD, COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, DT, OLD_DOF, NEW_DOF )
      ...
      DO I = 1, IMAX
          ...
          DO ORD = 0, COMMON_ORD
              ...
              TEMP = TEMP + OLD_DOF( ORD, I ) * POLY_C( ORD, 0.0D0 )
...
CONTAINS
  ...
  SUBROUTINE GET_BOUNDARY( MAX_ORD, I_BC, IMAX, DOF )

    IMPLICIT NONE

    INTEGER, INTENT( IN )      :: MAX_ORD
    INTEGER, INTENT( IN )      :: I_BC, IMAX
    REAL( 8 ), INTENT( INOUT ) :: DOF( 0 : MAX_ORD, - I_BC + 1 : IMAX + I_BC )
    ...
    DO I = 1, I_BC
        ...
        DO ORD = 0, MAX_ORD
            ...
            DOF( ORD, - I + 1 ) = DOF( ORD, IMAX - I + 1 )
            DOF( ORD, IMAX + I ) = DOF( ORD, I )
  ...
  ! 경계에서의 값도 업데이트 함. 
  SUBROUTINE GET_STAGE_DATA( POLY_ORD, COMMON_ORD, Q, I_BC, IMAX, X, DX, COM_DOF, LAMBDA, M_STAGE, L_B, R_B )
    ...
    REAL( 8 ), INTENT( IN )    :: X      ( - I_BC + 1 : IMAX + I_BC )
    REAL( 8 ), INTENT( IN )    :: DX     ( - I_BC + 1 : IMAX + I_BC )
    REAL( 8 ), INTENT( INOUT ) :: COM_DOF( 0 : COMMON_ORD, - I_BC + 1 : IMAX + I_BC )
    ...
    ELSEIF ( LAMBDA_SW .EQ. 1 ) THEN
        ...
        CALL GET_LAMBDA( COMMON_ORD, I_BC, IMAX, X, DX, COM_DOF, LAMBDA )
  ...
  SUBROUTINE PROJECT_COMMON_TO_ADAPTIVE( COMMON_ORD, POLY_ORD, Q, M_STAGE, I_BC, IMAX, COM_DOF, LAMBDA, ADP_DOF )
    ...
    REAL( 8 ), INTENT( IN )  :: COM_DOF( 0 : COMMON_ORD, - I_BC + 1 : IMAX + I_BC )
    REAL( 8 ), INTENT( IN )  :: LAMBDA ( - I_BC + 1 : IMAX + I_BC )

    REAL( 8 ), INTENT( OUT ) :: ADP_DOF( 0 : POLY_ORD, - I_BC + 1 : IMAX + I_BC )
        ...
        DO ORD = 0, POLY_ORD
            ...
            DO QUAD = 1, NQ
                ...
                DO K = 0, COMMON_ORD
                    ...
                    U_VAL = U_VAL + COM_DOF( K, I ) * POLY_C( K, Q( QUAD, 1 ) )

                END DO

                ADP_DOF( ORD, I ) = ADP_DOF( ORD, I ) &
                                  + U_VAL * POLY( ORD, Q( QUAD, 1 ), LAMBDA( I ) ) * Q( QUAD, 2 )

            END DO

            ADP_DOF( ORD, I ) = M_STAGE( ORD, ORD, I ) * ADP_DOF( ORD, I )

        END DO

    END DO

    CALL GET_BOUNDARY( POLY_ORD, I_BC, IMAX, ADP_DOF )
  ...
  !비디항식 기저--> 다항식 기저
  SUBROUTINE PROJECT_ADAPTIVE_TO_COMMON( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, ADP_DOF, LAMBDA, COM_DOF )
    ...
    REAL( 8 ), INTENT( IN )  :: ADP_DOF( 0 : POLY_ORD, - I_BC + 1 : IMAX + I_BC )
    REAL( 8 ), INTENT( IN )  :: LAMBDA ( - I_BC + 1 : IMAX + I_BC )

    REAL( 8 ), INTENT( OUT ) :: COM_DOF( 0 : COMMON_ORD, 1 : IMAX )
        ...
        DO K = 0, COMMON_ORD
            ...
            DO QUAD = 1, NQ
                ...
                DO ORD = 0, POLY_ORD
                    ...
                    U_VAL = U_VAL + ADP_DOF( ORD, I ) * POLY( ORD, Q( QUAD, 1 ), LAMBDA( I ) )

                END DO

                COM_DOF( K, I ) = COM_DOF( K, I ) &
                                + U_VAL * POLY_C( K, Q( QUAD, 1 ) ) * Q( QUAD, 2 )

            END DO

            COM_DOF( K, I ) = M( K, K ) * COM_DOF( K, I )
  ...
  SUBROUTINE GET_STAGE_RHS( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, COM_DOF, RHS_DOF )
    ...
    REAL( 8 ), INTENT( OUT )   :: RHS_DOF( 0 : COMMON_ORD, 1 : IMAX )

    ! CALCULATION VARIABLES
    REAL( 8 ), DIMENSION( 0 : POLY_ORD, - I_BC + 1 : IMAX + I_BC ) :: ADP_DOF
    REAL( 8 ), DIMENSION( 0 : POLY_ORD, 1 : IMAX )                 :: RHS_ADP
    !----------------------------------------- CALCULATIONS HAVE STARTED ----------------------------------------!

    ! DETERMINE THE CELL-WISE STAGE DATA
    CALL GET_STAGE_DATA( POLY_ORD, COMMON_ORD, Q, I_BC, IMAX, X, DX, COM_DOF, LAMBDA, M_STAGE, L_B, R_B )

    ! REPROJECT THE COMMON STAGE STATE ONTO THE ADAPTIVE STAGE BASIS
    CALL PROJECT_COMMON_TO_ADAPTIVE( COMMON_ORD, POLY_ORD, Q, M_STAGE, I_BC, IMAX, COM_DOF, LAMBDA, ADP_DOF )

    ! 여기서 RHS_ADP 이거 뭐지? 
    ! COMPUTE THE ADAPTIVE RESIDUAL
    CALL GET_RESIDUAL( POLY_ORD, Q, M_STAGE, L_B, R_B, I_BC, IMAX, DX, LAMBDA, ADP_DOF, RHS_ADP )
    ...
    ADP_DOF = RESHAPE_ADP( RHS_ADP, I_BC, IMAX, POLY_ORD ), LAMBDA = LAMBDA, COM_DOF = RHS_DOF )
  ...
  SUBROUTINE GET_ACCEPTED_STATE( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, COM_DOF )
    ...
    CALL PROJECT_ADAPTIVE_TO_COMMON( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, ADP_DOF, LAMBDA, TEMP_COM )
    ! 이거 왜 위에다가 com_DOF넣는 거 안하고 이렇게 따로 하는 걸까? 
    DO I = 1, IMAX

        DO ORD = 0, COMMON_ORD

            COM_DOF( ORD, I ) = TEMP_COM( ORD, I )

        END DO

    END DO

    CALL GET_BOUNDARY( COMMON_ORD, I_BC, IMAX, COM_DOF )
  ...
  SUBROUTINE GET_RK( RK_ORD, COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, DT, OLD_DOF, NEW_DOF )
    ...
    REAL( 8 ), INTENT( INOUT ) :: OLD_DOF( 0 : COMMON_ORD, - I_BC + 1 : IMAX + I_BC )
    REAL( 8 ), INTENT( OUT )   :: NEW_DOF( 0 : COMMON_ORD, 1 : IMAX )
    ...
    ! CALCULATION VARIABLES
    REAL( 8 ), DIMENSION( 0 : COMMON_ORD, - I_BC + 1 : IMAX + I_BC, 1 : 3 ) :: TEMP_DOF
    REAL( 8 ), DIMENSION( 0 : COMMON_ORD, 1 : IMAX )                        :: F_FLUX
    REAL( 8 ), DIMENSION( 0 : COMMON_ORD, - I_BC + 1 : IMAX + I_BC )        :: ACC_DOF
    !----------------------------------------- CALCULATIONS HAVE STARTED ----------------------------------------!
    ...
    IF ( RK_ORD .EQ. 3 ) THEN
        ...
        CALL GET_STAGE_RHS( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, OLD_DOF, F_FLUX )

        DO I = 1, IMAX

            DO ORD = 0, COMMON_ORD

              TEMP_DOF( ORD, I, 1 ) = + 1.0D0 * OLD_DOF( ORD, I ) &
        ...
        CALL GET_BOUNDARY( COMMON_ORD, I_BC, IMAX, TEMP_DOF( :, :, 1 ) )

        !==========================================================================================================!
        ! STAGE 2
        !==========================================================================================================!

        CALL GET_STAGE_RHS( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, TEMP_DOF( :, :, 1 ), F_FLUX )

        DO I = 1, IMAX

            DO ORD = 0, COMMON_ORD

              TEMP_DOF( ORD, I, 2 ) = + 3.0D0 * OLD_DOF( ORD, I ) &
                                      + 1.0D0 * TEMP_DOF( ORD, I, 1 ) &
                                      + 1.0D0 * DT * F_FLUX( ORD, I )

              TEMP_DOF( ORD, I, 2 ) = TEMP_DOF( ORD, I, 2 ) / 4.0D0

            END DO

        END DO

        CALL GET_BOUNDARY( COMMON_ORD, I_BC, IMAX, TEMP_DOF( :, :, 2 ) )

        !==========================================================================================================!
        ! STAGE 3
        !==========================================================================================================!

        CALL GET_STAGE_RHS( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, TEMP_DOF( :, :, 2 ), F_FLUX )

        DO I = 1, IMAX

            DO ORD = 0, COMMON_ORD

                NEW_DOF( ORD, I ) = + 1.0D0 * OLD_DOF( ORD, I ) &
                                    + 2.0D0 * TEMP_DOF( ORD, I, 2 ) &
                                    + 2.0D0 * DT * F_FLUX( ORD, I )

                NEW_DOF( ORD, I ) = NEW_DOF( ORD, I ) / 3.0D0
    ...
    ELSEIF ( RK_ORD .EQ. 4 ) THEN
        ...
        DO I = 1, IMAX
            ...
            DO ORD = 0, COMMON_ORD
                ...
                TEMP_DOF( ORD, I, 2 ) = + 1.0D0 * OLD_DOF( ORD, I ) &
        ...
        DO I = 1, IMAX
            ...
            DO ORD = 0, COMMON_ORD
                ...
                TEMP_DOF( ORD, I, 3 ) = + 1.0D0 * OLD_DOF( ORD, I ) &
                                        + 1.0D0 * DT * F_FLUX( ORD, I )
        ...
        CALL GET_BOUNDARY( COMMON_ORD, I_BC, IMAX, TEMP_DOF( :, :, 3 ) )

        !==========================================================================================================!
        ! STAGE 4
        !==========================================================================================================!

        CALL GET_STAGE_RHS( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, TEMP_DOF( :, :, 3 ), F_FLUX )
    ...
    DO I = 1, IMAX
        ...
        DO ORD = 0, COMMON_ORD
            ...
            ACC_DOF( ORD, I ) = NEW_DOF( ORD, I )

        END DO

    END DO

    CALL GET_BOUNDARY( COMMON_ORD, I_BC, IMAX, ACC_DOF )
    CALL GET_ACCEPTED_STATE( COMMON_ORD, POLY_ORD, Q, M, I_BC, IMAX, X, DX, ACC_DOF )

    DO I = 1, IMAX

        DO ORD = 0, COMMON_ORD

            NEW_DOF( ORD, I ) = ACC_DOF( ORD, I )
  ...
  SUBROUTINE GET_L2( I_BC, IMAX, DX, U_EXACT, U_NUM, ERR_L2 )
    ...
    DO I = 1, IMAX
        ...
        DO QUAD = 1, 6
            ...
            UH = 0.0D0
            DO ORD = 0, COMMON_ORD
                UH = UH + OLD_DOF( ORD, I ) * POLY_C( ORD, Q_ERR( QUAD, 1 ) )
            END DO
  ...
  ! @@@@@
    ...
    ! SUBROUTINE GET_LAMBDA( MAX_ORD, I_BC, IMAX, X, DX, DOF, LAMBDA )
    ...
    !     REAL( 8 )                  :: U_1
    !     REAL( 8 )                  :: U_3
    !     REAL( 8 )                  :: LAMBDA_S( - I_BC + 1 : IMAX + I_BC )
    !     !----------------------------------------- CALCULATIONS HAVE STARTED ----------------------------------------!

    !     CALL GET_BOUNDARY( MAX_ORD, I_BC, IMAX, DOF )

    !     ! COMPUTE CELL-WISE SHAPE PARAMETERS --> FEM METHOD
    !     DO I = 1, IMAX
    ...
    !             U_1 = U_1 + DOF( ORD, I ) * D_POLY_C ( ORD, 0.0D0 ) 
    !             U_3 = U_3 + DOF( ORD, I ) * D3_POLY_C( ORD, 0.0D0 ) 

    !         END DO

    !         IF( DABS( U_1 ) .LE. 1D-8 ) THEN
    ...
    !     IF ( IMAX .EQ. 256 .OR. IMAX .EQ. 300 ) THEN

    !         WRITE(*,*) '======================================='
    !         WRITE(*,*) '   LAMBDA COMPARISON (NUM vs EXACT)'
    !         WRITE(*,*) '======================================='
    !         WRITE(*,'(A)') '  I        lambda_num        lambda_exact       diff'
...
SUBROUTINE GET_LAMBDA( MAX_ORD, I_BC, IMAX, X, DX, DOF, LAMBDA )
  ...
  ! DERIVATIVES
  REAL( 8 )                 :: U1_L, U1_C, U1_R
  REAL( 8 )                 :: U3_L, U3_C, U3_R
  REAL( 8 )                 :: U1_BAR, U3_BAR
  ...
  DO I = 1, IMAX
      ...
      DO ORD = 0, MAX_ORD
          ...
          U1_L = U1_L + DOF( ORD, I ) * D_POLY_C ( ORD, -0.25D0 )
          U1_C = U1_C + DOF( ORD, I ) * D_POLY_C ( ORD,  0.0D0  )
          U1_R = U1_R + DOF( ORD, I ) * D_POLY_C ( ORD,  0.25D0 )

          U3_L = U3_L + DOF( ORD, I ) * D3_POLY_C( ORD, -0.25D0 )
          U3_C = U3_C + DOF( ORD, I ) * D3_POLY_C( ORD,  0.0D0  )
          U3_R = U3_R + DOF( ORD, I ) * D3_POLY_C( ORD,  0.25D0 )

      END DO

      ! weighted average (Simpson-like)
      U1_BAR = ( U1_L + 2.0D0 * U1_C + U1_R ) / 4.0D0
      U3_BAR = ( U3_L + 2.0D0 * U3_C + U3_R ) / 4.0D0

      ! lambda 계산
      IF ( DABS( U1_BAR ) .LE. 1.0D0-10 ) THEN
          LAMBDA(I) = 0.0D0
      ELSE
          LAMBDA(I) = U3_BAR / (4.0D0 * U1_BAR) 
      END IF
...
END SUBROUTINE GET_LAMBDA
  SUBROUTINE GET_RESIDUAL( POLY_ORD, Q, M_STAGE, L_B, R_B, I_BC, IMAX, DX, LAMBDA, DOF, F_FLUX )
    ...
    REAL( 8 ), INTENT( INOUT )                                     :: DOF    ( 0 : POLY_ORD, - I_BC + 1 : IMAX + I_BC )
    ...
    REAL( 8 ), DIMENSION( 0 : POLY_ORD, - I_BC + 1 : IMAX + I_BC ) :: DOF_REC
    !----------------------------------------- CALCULATIONS HAVE STARTED ----------------------------------------!
    ...
    IF (WENO_SW .EQ. 1) THEN
        ...
        CALL GET_DETECTOR( POLY_ORD, I_BC, IMAX, DOF, THETA )

        DO I = 1, IMAX

            IF ( THETA( I ) .GT. 1.0D0 ) THEN

                CALL GET_WENO( POLY_ORD, Q, M_STAGE, I_BC, IMAX, I, DOF, DOF_REC )
    ...
    CALL GET_BOUNDARY( POLY_ORD, I_BC, IMAX, DOF_REC )
    ...
    DO I = 1, IMAX
        ...
        DO ORD = 0, POLY_ORD
            ...
            DO QUAD = 1, NQ
                ...
                QUAD_SUM( QUAD ) = QUAD_SUM( QUAD ) + DOF_REC( ORD, I ) * POLY( ORD, Q( QUAD, 1 ), LAMBDA( I ) )
    ...
    ! FOR GIVEN DOFREES OF FREEDOM, COMPUTE CONVEX SUMMATIONS AT CELL INTERFACES
    DO I = 0, IMAX + 1
        ...
        DO ORD = 0, POLY_ORD
            ...
            ! LEFT BOUNDARY
            TEMP( 0 ) = TEMP( 0 ) + DOF_REC( ORD, I ) * L_B( ORD, I )

            ! RIGHT BOUNDARY
            TEMP( 1 ) = TEMP( 1 ) + DOF_REC( ORD, I ) * R_B( ORD, I )
  ...
  SUBROUTINE GET_WENO( POLY_ORD, Q, M_STAGE, I_BC, IMAX, TARGET_I, DOF_0, DOF )
    ...
    REAL( 8 ), INTENT( IN )                   :: Q( 1 : NQ, 1 : 2 )
    REAL( 8 ), INTENT( IN )                   :: M_STAGE( 0 : POLY_ORD, 0 : POLY_ORD, - I_BC + 1 : IMAX + I_BC )
    REAL( 8 ), INTENT( IN )                   :: DOF_0( 0 : POLY_ORD, - I_BC + 1 : IMAX + I_BC )
    ...
    DO L = - 1, 1
        ...
        DO QUAD = 1, NQ
            ...
            DO ORD = 0, POLY_ORD
                ...
                VAL   = VAL   + DOF_0( ORD, J ) * POLY   ( ORD, XI_J, LAMBDA( J ) )
                D1VAL = D1VAL + DOF_0( ORD, J ) * D_POLY ( ORD, XI_J, LAMBDA( J ) )
                D2VAL = D2VAL + DOF_0( ORD, J ) * D2_POLY( ORD, XI_J, LAMBDA( J ) )
    ...
    ! EVALUATE DOFREES OF FREEDOM (THE MOMENTS) BY USING THE MODIFIED DG POLYNOMIALS
    DO ORD = 1, POLY_ORD
        ...
        DOF( ORD, I ) = M_STAGE( ORD, ORD, I ) * VAL
  ...
  SUBROUTINE GET_DETECTOR( POLY_ORD, I_BC, IMAX, DOF, THETA )
    ...
    CALL GET_BOUNDARY( POLY_ORD, I_BC, IMAX, DOF )
    ...
    DO I = - I_BC + 1, IMAX + I_BC
        ...
        DO ORD = 0, POLY_ORD
            ...
            C_DX1_U( I ) = C_DX1_U( I ) &
                         + DOF( ORD, I ) * D_POLY ( ORD, 0.0D0, LAMBDA( I ) )

            C_DX2_U( I ) = C_DX2_U( I ) &
                         + DOF( ORD, I ) * D2_POLY( ORD, 0.0D0, LAMBDA( I ) )
